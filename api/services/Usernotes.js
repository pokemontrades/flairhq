...
exports.addUsernote = function (redToken, modname, subreddit, user, noteText, type, link_index) {
  return Reddit.getWikiPage(redToken, subreddit, 'usernotes').then(function (compressed_notes) {
    ...
    return Reddit.editWikiPage(redToken, subreddit, 'usernotes', JSON.stringify(parsed), 'FlairHQ: Created note on /u/' + user).then(function (resp) {
...
exports.removeUsernote = function (redToken, username, note_hash) {
  return Reddit.getWikiPage(redToken, subreddit, 'usernotes').then(function (compressed_notes) {
    ...
    return Reddit.editWikiPage(redToken, subreddit, 'usernotes', JSON.stringify(pageObject), reason).then(function (response) {
      return;
    }, function (err) {
      throw err;
    });
